...
class RouteService:
    ...
    def create_airport_node(self, airport):
    ...
    def add_flight_edge(self, flight: Flight):
    ...
    def sync_flight_created(self, flight: Flight):
        ...
        self.create_airport_node(flight.dep_airport)
        self.create_airport_node(flight.arr_airport)

        self.add_flight_edge(flight)

    def delete_flight_edge(self, dep_code: str, arr_code: str, flight_number: str):
        query = """
    ...
    def search_routes(self, session, from_city: str, to_city: str) -> list[Route]:

        from_airports = [a.code for a in session.query(City).filter_by(name=from_city).first().airports]
        to_airports = [a.code for a in session.query(City).filter_by(name=to_city).first().airports]
        ...
        for record in direct_results:
            numbers = record["flights"]

            flights = [
                session.query(Flight).filter_by(flight_number=n).first()
                for n in numbers
            ]
            routes.append(Route(flights))
